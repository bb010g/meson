...
class CMakeDependency(ExternalDependency):
    ...
    def _get_cmake_info(self, cm_args: T.List[str]) -> T.Optional[CMakeInfo]:
        ...
        for i in gen_list:
            ...
            # Run CMake
            gen_result = self._call_cmake(cmake_opts, 'CMakePathInfo.txt')
            ret1, out1, err1 = gen_result
        ...
        root_paths_set = process_paths(temp_parser.get_cmake_list_var(None, 'MESON_FIND_ROOT_PATH', [])[0])
        root_paths_set.update(process_paths(temp_parser.get_cmake_list_var(None, 'MESON_CMAKE_SYSROOT', [])[0]))
        root_paths = sorted(root_paths_set)
        root_paths = [x for x in root_paths if os.path.isdir(x)]
        module_paths_set = process_paths(temp_parser.get_cmake_list_var(None, 'MESON_PATHS_LIST', [])[0])
        ...
        archs, _ = temp_parser.get_cmake_list_var(None, 'MESON_ARCH_LIST', [])

        common_paths = ['lib', 'lib32', 'lib64', 'libx32', 'share']
        for i in archs:
            common_paths += [os.path.join('lib', i)]

        cmake_root, _ = temp_parser.get_cmake_str_var(None, 'MESON_CMAKE_ROOT', '')
    ...
    def _detect_dep(self, name: str, package_version: str, modules: T.List[T.Tuple[str, bool]], components: T.List[T.Tuple[str, bool]], args: T.List[str]) -> None:
        assert self.cmakebin is not None
        assert self.cmakeinfo is not None
        assert self.traceparser is not None
        ...
        self.is_found, _ = self.traceparser.get_cmake_bool_var(None, 'PACKAGE_FOUND')
        if not self.is_found:
            return

        # Try to detect the version
        vers_raw, _ = self.traceparser.get_cmake_str_var(None, 'PACKAGE_VERSION', '')

        if len(vers_raw) > 0:
            self.version = vers_raw.strip('"\' ')
        ...
        if len(modules) == 0:
            ...
            incDirs, _ = self.traceparser.get_cmake_list_var(None, 'PACKAGE_INCLUDE_DIRS', [])
            incDirs = [x for x in incDirs if x]
            defs, _ = self.traceparser.get_cmake_list_var(None, 'PACKAGE_DEFINITIONS', [])
            defs = [x for x in defs if x]
            libs_raw, _ = self.traceparser.get_cmake_list_var(None, 'PACKAGE_LIBRARIES', [])
            libs_raw = [x for x in libs_raw if x]
